...
namespace term
{
    ...
    public:
        constexpr font(void* notfound, uint8_t w, uint8_t h)
            : notfound(notfound), font_width(w), font_height(h), fonts{nullptr, nullptr, nullptr, nullptr}
        {
        }
        constexpr font(void* notfound, uint8_t w, uint8_t h, void* n, void* b, void* i, void* bi)
            : notfound(notfound), font_width(w), font_height(h), fonts{n, b, i, bi}
        {
        }
        constexpr font(const font& f)
            : fonts{f.fonts[0], f.fonts[1], f.fonts[2], f.fonts[3]}, notfound(f.notfound), font_width(f.font_width),
        ...
        constexpr size_t bpc() const
        {
            return font_width * font_height;
        }
        constexpr void set_font(void* font, style s)
        {
            fonts[s] = font;
        }

...
public:
    ...
    inline void copy(mregion& rhs, size_t n)
    {
        memcpy(rhs.buf, buf, n);
    }
    inline void set(uint8_t v, size_t n)
    {
        memset(buf, v, n);
    }
};
...
public:
    ...
    inline mbuffer(mbuffer&& rhs) : mregion(rhs.buf)
    {
        rhs.buf = nullptr;
    }
    inline ~mbuffer()
    ...
    inline void copy(mbuffer& rhs)
    {
        mregion::copy(rhs, min(n, rhs.n));
    }
